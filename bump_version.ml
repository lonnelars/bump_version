...
let print_usage () =
  print_endline
    "Usage: bump_version expects one command. The command must be major, \
     minor, or patch. It will read the current version from the file named \
     version, and write the new version to the same file."
...
let command_param = Command.Param.(anon ("command" %: string))

let main command =
  let open Option in
  let command = parse_command command in
  let version =
...
let command =
  Command.basic ~summary:"Increase version number in file `version`"
    (Command.Param.map command_param ~f:(fun command () -> main command))
